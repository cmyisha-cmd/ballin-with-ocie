...
app.post('/api/register', async (req,res)=>{
  ...
  if(shooting){
    const shootingList = await readJSON('shooting.json', [])
...
app.get('/api/shooting', async (_req,res)=>{
  const list = await readJSON('shooting.json', [])
...
app.patch('/api/shooting/:id', async (req,res)=>{
  ...
  let list = await readJSON('shooting.json', [])
...
app.get('/api/teams', async (_req,res)=>{
  const teams = await readJSON('teams.json', {A:[],B:[]})
  res.json(teams)
})
...
app.get('/api/tickets', async (_req,res)=>{
  const t = await readJSON('tickets.json', []); res.json(t)
})
app.post('/api/tickets', async (req,res)=>{
  const { name, quantity } = req.body || {}
  if(!name) return res.status(400).json({message:'Name required'})
  const t = await readJSON('tickets.json', [])
...
app.get('/api/messages', async (_req,res)=>{
  const m = await readJSON('messages.json', []); res.json(m)
})
app.post('/api/messages', async (req,res)=>{
  const { name, text } = req.body || {}
  if(!name || !text) return res.status(400).json({message:'Name and text required'})
  const m = await readJSON('messages.json', [])
