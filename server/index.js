...
async function readJSON(name, fallback){
  try { const t = await fs.readFile(f(name),'utf8'); return JSON.parse(t) }
  catch { return fallback }
}
async function writeJSON(name, val){
  await fs.mkdir(dataDir, {recursive:true})
  await fs.writeFile(f(name), JSON.stringify(val, null, 2))
}

// ---- Clean production (no seed data) ----
const empty = { players:[], shooting:[], teams:{A:[],B:[]}, tickets:[], messages:[] }

// Ensure files exist (first boot)
await writeJSON('players.json', await readJSON('players.json', empty.players))
await writeJSON('shooting.json', await readJSON('shooting.json', empty.shooting))
await writeJSON('teams.json', await readJSON('teams.json', empty.teams))
await writeJSON('tickets.json', await readJSON('tickets.json', empty.tickets))
await writeJSON('messages.json', await readJSON('messages.json', empty.messages))

app.get('/api/health', (_req,res)=>res.json({ok:true}))

// Players
app.get('/api/players', async (_req,res)=>{
  const players = await readJSON('players.json', empty.players); res.json(players)
})
...
app.post('/api/register', async (req,res)=>{
  ...
  // if shooting, add to shooting board
  if(shooting){
    const board = await readJSON('shooting.json', empty.shooting)
    board.push({ id, name, score:0, time:'00:00' })
    await writeJSON('shooting.json', board)
  }
...
app.patch('/api/shooting/:id', async (req,res)=>{
  const id = Number(req.params.id); const { score, time } = req.body || {}
  let list = await readJSON('shooting.json', empty.shooting)
  list = list.map(p => p.id===id ? {...p, score, time} : p)
  await writeJSON('shooting.json', list); res.json({ok:true})
})

// Teams
app.get('/api/teams', async (_req,res)=>{
  const teams = await readJSON('teams.json', empty.teams); res.json(teams)
})
...
// Messages (with reactions + replies)
...
app.post('/api/messages', async (req,res)=>{
  ...
  m.push({ id: Date.now(), name, text, reactions: {}, replies: [] })
  await writeJSON('messages.json', m); res.json({ok:true})
})
app.post('/api/messages/:id/react', async (req,res)=>{
  const id = Number(req.params.id); const { emoji } = req.body || {}
  const m = await readJSON('messages.json', empty.messages)
  const out = m.map(x => x.id===id ? {...x, reactions: {...x.reactions, [emoji]: (x.reactions?.[emoji]||0)+1 }} : x)
  await writeJSON('messages.json', out); res.json({ok:true})
})
app.post('/api/messages/:id/reply', async (req,res)=>{
  const id = Number(req.params.id); const { name, text } = req.body || {}
  const m = await readJSON('messages.json', empty.messages)
  const out = m.map(x => x.id===id ? {...x, replies:[...(x.replies||[]), {name, text}]} : x)
  await writeJSON('messages.json', out); res.json({ok:true})
})
