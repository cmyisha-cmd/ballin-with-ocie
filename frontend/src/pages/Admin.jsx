    ...
    <section className="grid" style={{margin:'28px 0'}}>
      ...
      <div className="card">
        ...
        <div className="grid" style={{marginTop:10}}>
          {Object.keys(teams).map(teamKey=>(
            <div key={teamKey}>
              <h4>Team {teamKey}</h4>
              <input type='text' id={`new-${teamKey}`} placeholder='New member name' />
              <button className='btn' onClick={()=>{
                const n=document.getElementById(`new-${teamKey}`).value;
                if(!n) return;
                fetch(`${API_URL}/api/teams/${teamKey}`,{
                  method:'POST',
                  headers:{'Content-Type':'application/json','x-admin-pass':'ocie2025'},
                  body:JSON.stringify({name:n})
                }).then(loadAll);
              }}>Add to Team {teamKey}</button>
              <ul>
                {teams[teamKey].map(t=>
                  <li key={t.id}>
                    {t.name}
                    <button className='btn danger' onClick={()=>{
                      fetch(`${API_URL}/api/teams/${teamKey}/${t.id}`,{
                        method:'DELETE',
                        headers:{'x-admin-pass':'ocie2025'}
                      }).then(loadAll);
                    }}>Delete</button>
                    <input type='text' defaultValue={t.name} id={`edit-${teamKey}-${t.id}`} />
                    <button className='btn' onClick={()=>{
                      const newName=document.getElementById(`edit-${teamKey}-${t.id}`).value;
                      fetch(`${API_URL}/api/teams/${teamKey}/${t.id}`,{
                        method:'PATCH',
                        headers:{'Content-Type':'application/json','x-admin-pass':'ocie2025'},
      ...
      <div className="card">
        ...
        <div className="cta" style={{display:'flex', gap:8, marginBottom:10}}>
          ...
          <a href="/bracket" className="btn">Open Bracket Page</a>
        </div>
